...
class NavigationNode(Node):
    ...
    def _control_loop(self):
        ...
        # CRITICAL: Do NOT fall back to odom frame - this causes frame mismatch!
        try:
            ...
            # Skip this control cycle - do NOT use odom frame as fallback
            # Using odom frame when path is in map frame causes wrong steering!
            self.get_logger().warn(
                f'Cannot get map->base_footprint TF: {e}. Skipping cycle.',
        ...
        # Debug logging for path following
        if hasattr(self, '_debug_counter'):
            self._debug_counter += 1
        else:
            self._debug_counter = 0

        if self._debug_counter % 40 == 0:  # Log every 2 seconds at 20Hz
            self.get_logger().info(
                f'Pose: ({x:.2f}, {y:.2f}, yaw={yaw:.2f}rad) '
    ...
    def _nav_path_to_pp_path(self, nav_path: NavPath) -> Path:
        ...
        # Validate frame - path must be in map frame
        if nav_path.header.frame_id != 'map':
            self.get_logger().warn(
                f'Path frame is "{nav_path.header.frame_id}", expected "map". '
                f'Navigation may be incorrect!'
            )
    ...
    def _compute_path_deviation(self) -> float:
        ...
        # Do NOT fall back to odom frame - return 0 if TF unavailable
        try:
            transform = self._tf_buffer.lookup_transform(
                'map', 'base_footprint',
                rclpy.time.Time(),
            ...
            # Return 0 instead of using wrong frame
            return 0.0
